...
//!
//! The hegel idioms it uses:
//!
//! - `#[hegel::test(settings())]` runs the body once per generated case. Each
//!   `tc.draw(g)` pulls one value from the generator `g`, and a failing case
//!   is shrunk and replayed.
//! - `#[hegel::composite]` turns `fn models(tc: &TestCase) -> Model` into
//!   `fn models() -> impl Generator<Model>`.
//! - On failure hegel prints every drawn value. `print_as_debug` makes the
//!   drawn `Model` appear in that report through `Debug`.
...
mod tests_feasibility {
    ...
    /// thin, and shallower runs mostly miss them. `HEGEL_TEST_CASES` in the
    /// environment overrides the count for a quicker run.
    ...
    /// The drawn model as data rather than a `Problem`, so that a failure can
    /// print the code that rebuilds it.
    #[derive(Debug)]
    struct Model {
    ...
    impl std::fmt::Display for Model {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            ...
            for (i, (obj, (lo, hi))) in self.reals.iter().enumerate() {
                writeln!(f, "let x{i} = problem.add_var({obj:?}, ({lo:?}, {hi:?}));")?;
            }
            if let Some((obj, (lo, hi))) = self.integer {
                writeln!(f, "problem.add_integer_var({obj:?}, ({lo:?}, {hi:?}));")?;
            }
    ...
    #[hegel::composite]
    fn models(tc: &hegel::TestCase) -> Model {
        ...
        // of zero, so variables fixed at zero by their own bounds and models
        // whose only feasible point is the origin (as in #44) are common.
        ...
        // dropped, so a model can have fewer.
    ...
    /// inequality's rhs has the sign that leaves the origin slack, so the
    /// origin satisfies every bound and every row. Every bound is finite, so no
    ...
    fn an_origin_feasible_bounded_model_solves(tc: hegel::TestCase) {
        let model = tc.draw(models().print_as_debug());
